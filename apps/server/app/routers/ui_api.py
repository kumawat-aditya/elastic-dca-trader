...
async def dashboard_stream(websocket: WebSocket):
    ...
    logger.info("New UI client connected to Dashboard WebSocket stream.")
    try:
        while True:
            # Send the complete state as JSON
            state_dump = engine.state.model_dump()
        ...
        logger.info("UI Dashboard WebSocket Disconnected.")
    except Exception as e:
        logger.error(f"UI WebSocket Error: {e}", exc_info=True)
...
def acknowledge_alert(side: str, index: int):
    ...
    if side not in ["buy", "sell"]: 
        logger.warning(f"Invalid side '{side}' passed to ack-alert.")
    ...
    for row in settings.rows:
        if row.index == index:
            ...
            logger.debug(f"[{side.upper()}] Alert acknowledged for row index {index}.")
            return {"status": "success"}
            
    logger.warning(f"[{side.upper()}] Failed to ack alert: Row {index} not found.")
...
def toggle_grid_state(side: str, payload: ControlPayload):
    ...
    if side not in ["buy", "sell"]:
        logger.warning(f"Invalid side '{side}' passed to control.")
        raise HTTPException(status_code=400, detail="Invalid side")
    ...
    if grid_settings.is_on and not payload.is_on:
        ...
        if grid_state.session_id:
            ...
            logger.info(f"[{side.upper()}] Grid manually turned OFF by User. Sent CLOSE_ALL.")
        else:
            logger.info(f"[{side.upper()}] Grid manually turned OFF by User. No active session to close.")
    ...
    # If turning ON, Engine will catch it on the next tick and initiate the anchor
        ...
        logger.info(f"[{side.upper()}] Grid manually turned ON by User. Awaiting anchor/crossover.")
...
def update_grid_settings(side: str, new_settings: GridSettings):
    ...
    """
    if side not in["buy", "sell"]:
        logger.warning(f"Invalid side '{side}' passed to settings update.")
        raise HTTPException(status_code=400, detail="Invalid side")
    ...
    if current_settings.is_on:
        ...
        for old_idx, old_row in old_executed_map.items():
            if old_idx not in new_rows_map:
                logger.warning(f"[{side.upper()}] UI attempted to delete executed row {old_idx}. Rejected.")
            ...
            if new_row.gap != old_row.gap or new_row.lots != old_row.lots:
                logger.warning(f"[{side.upper()}] UI attempted to alter gap/lots of executed row {old_idx}. Rejected.")
    ...
    logger.info(f"[{side.upper()}] Grid settings successfully updated by User.")
...
def load_preset(preset_id: int, side: str, db: Session = Depends(get_db)):
    ...
    """
    if side not in ["buy", "sell"]:
        logger.warning(f"Invalid side '{side}' passed to preset load.")
        raise HTTPException(status_code=400, detail="Invalid side")
        
    current_settings = engine.state.buy_settings if side == "buy" else engine.state.sell_settings
    if current_settings.is_on:
        logger.warning(f"[{side.upper()}] UI attempted to load preset while grid is ON. Rejected.")
    ...
    if not preset:
        logger.warning(f"UI attempted to load non-existent preset ID: {preset_id}. Rejected.")
    ...
    engine.recalculate_grid_math(side)
    logger.info(f"[{side.upper()}] Preset '{preset.name}' successfully loaded into grid.")
