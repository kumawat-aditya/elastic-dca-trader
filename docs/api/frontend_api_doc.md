...
* **Unidirectional Data Flow:** The WebSocket provides the absolute truth. When a user changes settings, send a REST request. Do *not* update the UI manually—wait for the backend to recalculate and push the updated state via the WebSocket.
...
```
**Frontend Handling:**
* If the user clicks the "Power" toggle to turn the grid OFF, warn them: *"Turning off will immediately close all open trades for this grid. Proceed?"*
...
2. In your rendering loop for the rows:
   ...
   />
   ```
3. The user can safely edit the `gap`/`lots` of *unexecuted* rows, and can edit `start_limit`, `TP`, `SL`, and `alert` checkboxes at any time.
...
state.buy_settings.rows.forEach(row => {
  if (row.alert === true && row.executed === true && row.alert_executed === false) {
...
* **Frontend Handling:** 
  ...
  * If the user clicks it, show a tooltip: *"You must turn the grid OFF before applying a preset."*
...
*   **`400 Bad Request`**: 
    *   *Cause A:* You tried to change the gap/lots of an `executed` row.
    *   *Cause B:* You tried to load a preset while the grid was `is_on = true`.
    *   *Cause C:* You tried to save a preset name that already exists.
*   **`404 Not Found`**:
    *   *Cause A:* You sent an `ack-alert` for an index that doesn't exist.
    *   *Cause B:* You tried to load a preset ID that was deleted.
*   **`422 Unprocessable Entity`**:
    *   *Cause:* Your JSON payload did not perfectly match the TypeScript interfaces defined above (e.g., you sent a string instead of a float for a gap).